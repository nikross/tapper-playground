...
export const laterpayMockClient = new ClientOAuth2({
  clientId: process.env.LP_CONFIDENTIAL_CLIENT_ID, // Client app with token_endpoint_auth_method set to 'client_secret_basic'
  clientSecret: process.env.LP_CONFIDENTIAL_CLIENT_SECRET,
  accessTokenUri: `${process.env.NEXT_PUBLIC_BASE_URL}/api/stubs/hydra/token`,
  authorizationUri: `${process.env.NEXT_PUBLIC_BASE_URL}/api/stubs/hydra/auth`,
  redirectUri: `${process.env.NEXT_PUBLIC_BASE_URL}/api/auth/callback`,
  scopes: ['read', 'write'],
  state: 'staticState' // nanoid()
})
